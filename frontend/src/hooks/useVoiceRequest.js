...
const parseTranscript = (text) => {
    ...
    // Handles: "O positive", "O negative", "A +", "B minus"
    let bloodGroup = '';
    const bgMap = {
    ...
    // Check phrasal matches first (longer strings first)
    ...
    // Fallback to simple patterns if no phrase match
    if (!bloodGroup) {
        const simpleGroups = ['ab+', 'ab-', 'a+', 'a-', 'b+', 'b-', 'o+', 'o-'];
        for (const bg of simpleGroups) {
            // Check for exact symbol match or space separating (e.g. "a +")
    ...
    if (
        ...
        lower.includes('procedure') || // often requested for blood
    ...
    let resourceType = 'blood'; // Default
    let organType = '';
    ...
    // 5. Quantity Parsing
    // Match "2 units", "two units", "2 bags", "3 bottles", "one unit"
    let quantity = 1;
    ...
    // 6. Patient Name Parsing (Improved)
    let patientName = '';

    // Strategy A: "Patient [Name]" or "Patient name [Name]"
    // Stop capturing if we hit typical "end of name" verbs/prepositions like "is", "needs", "requires", "suffering", "in"
    const nameStopWords = ['is', 'needs', 'requires', 'has', 'suffering', 'demands', 'wants', 'urgently', 'immediately', 'due', 'for', 'with', 'in'];

    // Regex try 1: Explicit markers
    ...
    if (explicitNameMatch && explicitNameMatch[1]) {
        ...
        // Check if the second word is actually a stop word (e.g. "Patient John needs") -> captured "John needs"
    ...
    // Clean up name (remove trailing stop words if regex missed them)
    if (patientName) {
        const nameParts = patientName.split(/\s+/);
        // If last part is a stop word, remove it
